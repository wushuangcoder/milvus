	...
	blob, err := proto.Marshal(&planpb.PlanNode{
		Node: &planpb.PlanNode_VectorAnns{
			VectorAnns: &planpb.VectorANNS{
				...
				QueryInfo:  &planpb.QueryInfo{Topk: topk, MetricType: "L2", SearchParams: "{}"},
			},
		},
	})
	require.NoError(t, err)
...
func TestOptimizeSharedFilterBranchesStage2IsPerBranch(t *testing.T) {
	...
	hook.On("Run", mock.Anything).Run(func(args mock.Arguments) {
		...
		withFilter = append(withFilter, params[common.WithFilterKey].(bool))
	}).Return(nil)
	...
	sd := &shardDelegator{}
	require.NoError(t, sd.optimizeSharedFilterBranches(context.Background(), req, hook, 5, true, dim))

	// Order follows the extras: BM25 keeps its filter, the qualifying dense
	// branch takes stage-2 semantics, the small-topk dense branch keeps its
	// filter too.
	assert.Equal(t, []bool{true, false, true}, withFilter)
}
...
// Vector-clustering-key pruning reads branch 0's query vector and rewrites the
// shared segment list by it, which is wrong for every other branch. It must be
// skipped for a group; scalar-key pruning reads the shared predicate and stays.
func TestGroupedRequestSkipsVectorPrune(t *testing.T) {
	paramtable.Init()
	...
	grouped := &querypb.SearchRequest{
		Req:                    &internalpb.SearchRequest{},
		ExtraFilterSharingReqs: []*internalpb.SubSearchRequest{{}},
	}
	single := &querypb.SearchRequest{Req: &internalpb.SearchRequest{}}

	assert.True(t, groupedRequestSkipsVectorPrune(grouped, vectorKey), "group + vector key: skip")
	assert.False(t, groupedRequestSkipsVectorPrune(grouped, scalarKey), "group + scalar key: predicate is shared, prune")
	assert.False(t, groupedRequestSkipsVectorPrune(grouped, noKey), "group + no key: nothing to skip")
	assert.False(t, groupedRequestSkipsVectorPrune(single, vectorKey), "single branch: its own vector, prune")
}
