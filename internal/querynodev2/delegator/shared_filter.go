...
// re-marshalling its predicate and hashing it, once per sub-request and
...
func (sd *shardDelegator) optimizeSharedFilterBranches(
	...
	for i, sub := range req.GetExtraFilterSharingReqs() {
		...
		// Stage 2 tells the query hook the filter's selectivity is already
		// known (WithFilterKey=false) and suppresses topk-reduce. That is only
		// true of a branch two-stage search was built for; a co-grouped branch
		// that would not have qualified on its own -- a BM25 branch next to a
		// dense one -- must keep its ordinary parameters even though it shares
		// the group's stage-1 bitset.
		branchStage2 := isSecondStageSearch &&
			branchQualifiesForTwoStage(sub.GetTopk(), sub.GetSearchType())
		optimized, err := optimizers.OptimizeSearchParams(ctx, branchReq, queryHook, numSegments, branchStage2, dimFunc)
		if err != nil {
			return err
		}
...
// groupedRequestSkipsVectorPrune reports whether segment pruning must be
// skipped for a shared-filter group.
//
// PruneSegments on a vector clustering key reads the query vector out of the
// request's placeholder group -- branch 0's -- and writes the surviving
// segments back into the shared snapshot. Every other branch would then
// search a segment set pruned by a vector that is not its own, and a BM25
// branch next to a dense one would silently lose rows. Scalar-key pruning
// reads the predicate, which the group shares by construction, so it stays.
func groupedRequestSkipsVectorPrune(req *querypb.SearchRequest, schema *schemapb.CollectionSchema) bool {
