...
func (sd *shardDelegator) search(ctx context.Context, req *querypb.SearchRequest, sealed []SnapshotItem, growing []SegmentEntry, sealedRowCount map[int64]int64) ([]*internalpb.SearchResults, error) {
	...
	if paramtable.Get().QueryNodeCfg.EnableSegmentPrune.GetAsBool() {
		if groupedRequestSkipsVectorPrune(req, sd.collection.Schema()) {
			// Pruning by branch 0's query vector would be wrong for every
			// other branch; see groupedRequestSkipsVectorPrune.
			mlog.Debug(ctx, "skip vector-clustering-key segment pruning for a shared-filter group",
				mlog.Int("branches", len(req.GetExtraFilterSharingReqs())+1))
		} else {
			func() {
				sd.partitionStatsMut.RLock()
				defer sd.partitionStatsMut.RUnlock()
