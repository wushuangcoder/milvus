...
treats as unshareable — the feature is inert, never wrong.
...
turn it on only once every query node is upgraded.
...
state. That holds because the two-phase segcore path is only entered for a group of two or more:
...
One request-scoped decision reads two of those and had to be carved out. With
`queryNode.enableSegmentPrune=true` and a **vector** clustering key (itself gated by
`common.enableVectorClusteringKey`), `PruneSegments` takes the query vector out of the request's
`placeholder_group` — branch 0's — computes centroid distances with the request's `metricType`, and
writes the surviving segments back into the shared snapshot. Every other branch would then search a
segment set pruned by a vector that is not its own; a BM25 branch next to a dense one would silently
lose rows. For a grouped request, vector-key pruning is therefore **skipped** and the group searches
the full snapshot (`groupedRequestSkipsVectorPrune`). Scalar-key pruning reads the predicate, which the
group shares by construction, and stays. Pruning per branch and searching the union would keep the
benefit; it is a follow-up because `PruneSegments` mutates the snapshot in place.
...
- `IsTopkReduce` / `IsRecallEvaluation` are **OR-ed** across branches into the envelope at the
  delegator (`optimizeSharedFilterBranches`), so the proxy's top-level `resultSizeInsufficient &&
...
- `ScannedRemoteBytes` / `ScannedTotalBytes` are **summed** over every branch's segment results in the
  worker (`attributeStorageCost`), so the envelope carries the true group total; the demux attaches it
  to branch 0 and zeroes the rest, and the top-level sum is unchanged. These feed
  `milvus_proxy_scanned_{remote,total}_mb`.
- `TotalRelatedDataSize` is the size of the segments a request touched, not work done. Today each
  ...
  segment twice. A group reports it **once**. This feeds the `related_data_size` metering hook, so it
  is a visible change in that figure for grouped searches, in the direction of counting each segment
  once.
- `ResponseTime` / `ServiceTime` / `TotalNQ` are merged by **maximum** (`mergeRequestCost`), so zeros
  on the non-zero branches are inert.
